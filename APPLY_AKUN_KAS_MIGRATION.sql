-- ================================================
-- APPLY AKUN KAS MIGRATION
-- ================================================
-- Run this script in Supabase SQL Editor to fix the akun_kas issues
-- ================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS akun_kas_nama_unique_active 
ON akun_kas(nama) 
WHERE status = 'aktif';

-- Create partial unique constraint for kode (only for active accounts)
CREATE UNIQUE INDEX IF NOT EXISTS akun_kas_kode_unique_active 
ON akun_kas(kode) 
WHERE status = 'aktif';
...
WHERE routine_schema = 'public' 
  AND routine_name IN ('check_deleted_account_exists', 'restore_deleted_account');
