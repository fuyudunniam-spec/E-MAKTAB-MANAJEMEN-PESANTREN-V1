...
export class UserService {
  ...
  static async createSantriAccount(input: CreateSantriAccountInput): Promise<{ userId: string; email: string; password: string }> {
    ...
    const { createSantriAccount } = await import('@/services/santriAuth.service');
    ...
    if (!password && santri.tanggal_lahir) {
      ...
      const { UserManagementService } = await import('@/services/userManagement.service');
...
export type { AppRole } from '@/services/userManagement.service';
