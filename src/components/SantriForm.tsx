...
const SantriForm: React.FC<SantriFormProps> = ({ santriId, onClose, onSave }) => {
  ...
  useEffect(() => {
    ...
    console.log('useEffect triggered - santriId:', santriId, 'kategori:', santriData.kategori);
    console.log('Dependencies changed - will call loadSantriData:', !!santriId);
    if (santriId) {
      console.log('Calling loadSantriData...');
      loadSantriData();
    } else {
      console.log('Calling updateDokumenDasar...');
      // Set dokumen dasar sesuai kategori
      updateDokumenDasar();
    }
  }, [santriId, santriData.tanggal_lahir]); // Removed santriData.kategori from dependencies

  // Update documents when kategori or status anak changes (only for new santri)
  useEffect(() => {
    console.log('=== KATEGORI/STATUS CHANGE USEEFFECT ===');
    console.log('Kategori:', santriData.kategori, 'Status Anak:', santriData.status_anak, 'santriId:', santriId);
    if (!santriId) {
      console.log('Updating documents for new santri...');
      updateDokumenDasar();
    }
  }, [santriData.kategori, santriData.status_anak]);
  ...
  const loadSantriData = async () => {
    ...
    try {
      ...
      console.log('=== LOAD SANTRI DATA DEBUG ===');
      console.log('Loading santri data - original kategori:', santri.kategori);
      console.log('Will set kategori to:', santri.kategori || 'Reguler');
      const newSantriData = {
        ...santri,
      ...
      console.log('Setting santriData with kategori:', newSantriData.kategori);
      setSantriData(newSantriData);
  ...
  const updateDokumenDasar = async () => {
    try {
      console.log('=== UPDATE DOKUMEN DASAR DEBUG ===');
      console.log('Kategori:', santriData.kategori);
      console.log('Status Anak:', santriData.status_anak);
      console.log('Tanggal Lahir:', santriData.tanggal_lahir);
      
      // Get wali utama untuk menentukan KTP yang dibutuhkan
      const waliUtama = waliData.find(w => w.is_utama);
      console.log('Wali Utama:', waliUtama);
      
      // Use hardcoded document requirements since RPC function doesn't exist
      let requiredDocs = null;
      if (santriData.tanggal_lahir) {
        // Base documents required for all santri
        const baseDocs = [
          { jenis_dokumen: 'KK' },
          { jenis_dokumen: 'Akta Lahir' },
          { jenis_dokumen: 'Pas Foto' }
        ];
        
        // Add conditional documents based on category
        if (santriData.kategori === 'Binaan Mukim' || santriData.kategori === 'Binaan Non-Mukim') {
          baseDocs.push({ jenis_dokumen: 'Surat Domisili' });
          console.log('Added Surat Domisili for Binaan category');
        }
        
        // Add documents based on status anak
        if (santriData.status_anak === 'Yatim' || santriData.status_anak === 'Piatu' || santriData.status_anak === 'Yatim Piatu') {
          // Akta Kematian menjadi wajib untuk status yatim/piatu
          if (santriData.status_anak === 'Yatim' || santriData.status_anak === 'Yatim Piatu') {
            baseDocs.push({ jenis_dokumen: 'Akta Kematian Ayah' });
          }
          if (santriData.status_anak === 'Piatu' || santriData.status_anak === 'Yatim Piatu') {
            baseDocs.push({ jenis_dokumen: 'Akta Kematian Ibu' });
          }
          console.log('Added Akta Kematian for Yatim/Piatu status');
        }
        
        // SKTM wajib untuk Dhuafa
        if (santriData.status_anak === 'Dhuafa') {
          baseDocs.push({ jenis_dokumen: 'SKTM' });
          console.log('Added SKTM for Dhuafa status');
        }
        
        // Add KTP based on age (simplified calculation)
        const birthYear = new Date(santriData.tanggal_lahir).getFullYear();
        const currentYear = new Date().getFullYear();
        const age = currentYear - birthYear;
        console.log('Age calculation:', { birthYear, currentYear, age });
        
        if (age >= 17) {
          baseDocs.push({ jenis_dokumen: 'KTP Santri' });
          console.log('Added KTP Santri for age >= 17');
        }
        
        // Add wali KTP based on relationship
        if (waliUtama?.hubungan_keluarga) {
          const waliKTP = `KTP ${waliUtama.hubungan_keluarga}`;
          baseDocs.push({ jenis_dokumen: waliKTP });
          console.log('Added Wali KTP:', waliKTP);
        }
        
        requiredDocs = baseDocs;
        console.log('Final required documents:', requiredDocs);
      }
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Call onSave immediately to refresh data
      onSave();
      
      // Close form after a short delay to ensure data is refreshed
      setTimeout(() => {
        onClose();
      }, 500);
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => {
                        console.log('=== KATEGORI CHANGE DEBUG ===');
                        console.log('Current kategori:', santriData.kategori);
                        console.log('New value:', value);
                        console.log('Current santriData:', santriData);
                        console.log('Setting new state...');
                        setSantriData({...santriData, kategori: value});
            ...
            <TabsContent value="dokumen" className="space-y-4">
              {santriId ? (
                <UploadDokumenSantri
                  ...
                  onUploadComplete={() => {
                    console.log('Document upload completed, refreshing parent data');
                    // Trigger parent refresh to update document completeness
                    onSave();
                  }}
                />
              ) : (
